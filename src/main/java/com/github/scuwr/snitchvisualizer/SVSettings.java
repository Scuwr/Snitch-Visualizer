...
public class SVSettings{
    ...
    public String getKeyBinding(Options option){
        String s = (new StringBuilder()).append(StatCollector.translateToLocal(option.getEnumString())).append(": ").toString();
        ...
        if (option.getEnumFloat()){
            ...
            if (option == Options.RENDER_DISTANCE){
                if (f == option.getValueMin()){
                    return (new StringBuilder()).append(s).append(StatCollector.translateToLocal("MIN")).toString();
                }

                if (f == option.getValueMax()){
                    return (new StringBuilder()).append(s).append(StatCollector.translateToLocal("MAX")).toString();
                }
                else{
                    return (new StringBuilder()).append(s).append((int)(f)).append(" chunks").toString();
                }
            }
            else if (option == Options.RENDER_ENABLED){
            	if (b == true){
            		return (new StringBuilder()).append(s).append(StatCollector.translateToLocal("options.on")).toString();
            	}
            	else{
            ...
            if (f == 0.0F){
                ...
                return (new StringBuilder()).append(s).append((int)(f)).append("options.on").toString();
            }
        }
        else{
            return s;
	...
	public enum Options{	
	    ...
	    private Options(String s1, boolean flag, boolean flag1, float valueMin, float valueMax, float valueStep){
	    	this.enumString = s1;
	        this.enumFloat = flag;
	        this.enumBoolean = flag1;
